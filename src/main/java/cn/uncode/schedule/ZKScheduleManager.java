...
public class ZKScheduleManager extends ThreadPoolTaskScheduler implements ApplicationContextAware {
	...
	public void destroy(){
		...
		if(DynamicTaskManager.SCHEDULE_FUTURES != null && !DynamicTaskManager.SCHEDULE_FUTURES.isEmpty()){
			Set<String> keySet = DynamicTaskManager.SCHEDULE_FUTURES.keySet();
			ArrayList<String> list = new ArrayList<>(keySet);
			DynamicTaskManager.clearLocalTask(list);
		}
	...
	class HeartBeatTimerTask extends java.util.TimerTask {
		...
		public void run() {
			try {
				logger.info("=========UncodeSchedule HeartBeatTimerTask()=============");
	...
	class InitialThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				System.out.println("===============sm.initialData();==========");
				sm.initialData();
			} catch (Throwable e) {
	...
	private void addTask(Runnable task, TaskDefine taskDefine){
		if(task instanceof org.springframework.scheduling.support.ScheduledMethodRunnable && scheduleDataManager !=null){
			try {
				scheduleDataManager.addTask(taskDefine);
			} catch (Exception e) {
				LOGGER.info(String.format("add task exception, taskName:%s", taskDefine.stringKey()), e);
			}
		}
	}
